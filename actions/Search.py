...
class Graph:
    ...
    def GreedyAlgorithm(self, st, en):
            ...
            if current_node == en: #ket thuc
                ...
                for i in path:
                    ...
                    if path.index(i) != len(path) - 1: #not the last
                        result += ' <-- '
                return result + '; Chi phí là: ' + str(cost)
        ...
        return 'Không tim thấy đường đi'
        ...
        while len(queue_node):
            ...
            if current_node == en: #ket thuc
                ...
                for i in path:
                    ...
                    if path.index(i) != len(path) - 1: #not the last
                        result += ' <-- '
                return result + '; Chi phí là: ' + str(total_cost)
...
def test():      
    a = Graph(['A', 'B', 'C', 'D', 'E', 'F', 'G','H', 'I', 'K'], [123,82,118,115,72,40,0,70,40,30])
    a.addNode('S', 125)
    a.addEdge('S', 'A', 55)
    a.addEdge('S', 'B', 42)
    a.addEdge('S', 'C', 48)
